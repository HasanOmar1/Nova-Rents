...
export default function ReportedUsers() {
  ...
  const changeStatus = async (user, block) => {
    ...
    finally { setModal(null); setLoadingAction(""); }
  };
  const removeLatestWarning = async (user) => {
    ...
    try {
      ...
      setModal(null); setLoadingAction("");
    }
  };
  ...
  return <div className={`${styles.page} page`}>
    ...
    <section className={styles.tableWrap}><div className={styles.scroll}><table><thead><tr><th>Email</th><th>Direct</th><th>Vehicle</th><th>Total</th><th>Open</th><th>Warnings</th><th>Status</th><th>Last report</th><th>Risk</th><th>Actions</th></tr></thead>
      <tbody>{users.map(user=><tr key={user.userId}><td className={styles.emailCell}>{user.email}</td><td>{user.directReports}</td><td>{user.vehicleReports}</td><td>{user.totalReports}</td><td>{user.openReports}</td>
        ...
        <td><div className={styles.actions}>
          <AsyncButton className={styles.viewAction} title="View reports" aria-label="View reports" loading={loadingAction===`reports-${user.userId}`} loadingText="" onClick={()=>openDetails(user,"reports")}><Eye size={16}/></AsyncButton>
          <AsyncButton className={styles.warnAction} title="Warn user" aria-label="Warn user" disabled={Number(user.warningCount)>=3} onClick={()=>{setReason("");setWarningError("");setModal({type:"warn",user})}}><TriangleAlert size={16}/></AsyncButton>
          <AsyncButton className={styles.removeWarningAction} title="Remove latest warning" aria-label="Remove latest warning" disabled={Number(user.warningCount)===0} onClick={()=>setModal({type:"confirmRemoveWarning",user})}><Undo2 size={16}/></AsyncButton>
          <AsyncButton className={user.status==="blocked"?styles.unblockAction:styles.blockAction} title={user.status==="blocked"?"Unblock user":"Block user"} aria-label={user.status==="blocked"?"Unblock user":"Block user"} onClick={()=>setModal({type:"confirmStatus",user,block:user.status!=="blocked"})}>{user.status==="blocked"?<ShieldCheck size={16}/>:<Ban size={16}/>}</AsyncButton>
    ...
    {modal && <div className={styles.overlay} onMouseDown={()=>setModal(null)}><div className={styles.modal} onMouseDown={(e)=>e.stopPropagation()}><button className={styles.close} onClick={()=>setModal(null)} aria-label="Close"><X/></button>
      ...
      {modal.type==="confirmWarning" && <div className={styles.confirmContent}><div className={styles.confirmIcon}><TriangleAlert size={28}/></div><p>{Number(modal.user.warningCount)===2?"This is the user's third warning. Issuing it will automatically block the account.":"Are you sure you want to issue this official warning?"}</p><div className={styles.reasonPreview}><span>Reason</span><p>{reason.trim()}</p></div>{warningError&&<p className={styles.modalError}><AlertTriangle size={16}/>{warningError}</p>}<div className={styles.modalActions}><button className={styles.cancelButton} disabled={Boolean(loadingAction)} onClick={()=>setModal({type:"warn",user:modal.user})}>Go back</button><AsyncButton className={styles.warningConfirmButton} loading={loadingAction===`warn-${modal.user.userId}`} loadingText="Issuing..." onClick={issueWarning}>Issue warning</AsyncButton></div></div>}
      {modal.type==="confirmStatus" && <div className={styles.confirmContent}><div className={`${styles.confirmIcon} ${modal.block?styles.dangerIcon:styles.successIcon}`}>{modal.block?<Ban size={28}/>:<ShieldCheck size={28}/>}</div><p>{modal.block?"This user will be blocked and will no longer be able to access restricted Nova Rents features.":"This user will regain access to Nova Rents."}</p><div className={styles.modalActions}><button className={styles.cancelButton} disabled={Boolean(loadingAction)} onClick={()=>setModal(null)}>Cancel</button><AsyncButton className={modal.block?styles.dangerButton:styles.successButton} loading={loadingAction===`status-${modal.user.userId}`} loadingText={modal.block?"Blocking...":"Unblocking..."} onClick={()=>changeStatus(modal.user,modal.block)}>{modal.block?"Block account":"Unblock account"}</AsyncButton></div></div>}
      {modal.type==="confirmRemoveWarning" && <div className={styles.confirmContent}><div className={styles.removeIcon}><Undo2 size={28}/></div><p>This will hide the user's most recent warning while retaining its audit record. The account will remain {modal.user.status}.</p><div className={styles.modalActions}><button className={styles.cancelButton} disabled={Boolean(loadingAction)} onClick={()=>setModal(null)}>Cancel</button><AsyncButton className={styles.removeWarningButton} loading={loadingAction===`remove-${modal.user.userId}`} loadingText="Removing..." onClick={()=>removeLatestWarning(modal.user)}>Remove warning</AsyncButton></div></div>}
