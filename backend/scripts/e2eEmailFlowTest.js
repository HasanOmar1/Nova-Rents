...
(async () => {
  ...
  console.log(`setup: vehicle plate=${licensePlate} (${model.brandName} ${model.modelName})`);
  ...
  assert(create.status === 201, `rental request created (status ${create.status})`);
  assert(Number.isInteger(create.data.rentalId), `response returns rentalId (${create.data.rentalId})`);
  assert(create.data.emailSent === true, "response reports emailSent: true (owner request email)");
  const rentalId = create.data.rentalId;

  const rentalRows = await doQuery("SELECT * FROM rentals WHERE rentalId = ?", [rentalId]);
  ...
  assert(ownerCheck.userId === ownerId, "owner resolved via plate join is User B");
  assert(ownerCheck.email === EMAIL_B, "resolved recipient email is User B's email (not requester)");

  // ---------- 2. Invalid + overlapping requests send no email ----------
  const before = await doQuery("SELECT COUNT(*) AS n FROM rentals WHERE licensePlate = ?", [licensePlate]);
  const invalid = await api(cookieA, "POST", "/rentals/rent", {
    licensePlate,
    startDate: "2020-01-01",
    endDate: "2020-01-02",
  });
  assert(invalid.status === 400, `invalid request rejected (status ${invalid.status})`);
  assert(invalid.data.emailSent === undefined, "invalid request response has no emailSent (no email path reached)");
  ...
  assert(overlap.status === 400, `overlapping request rejected (status ${overlap.status})`);
  const after = await doQuery("SELECT COUNT(*) AS n FROM rentals WHERE licensePlate = ?", [licensePlate]);
  assert(before[0].n === after[0].n, "no extra rental rows from invalid/overlap attempts");

  // ---------- 3. Approval -> payment link email to requester ----------
  const approve = await api(cookieB, "PUT", `/rentals/approve-rental/${rentalId}`);
  assert(approve.status === 200, `approval succeeded (status ${approve.status})`);
  ...
  assert(payment.paymentLinkEmailSentAt !== null, "paymentLinkEmailSentAt recorded (payment email sent)");

  // ---------- 4. Mock payment link works for the requester ----------
  const view = await api(cookieA, "GET", `/payments/${payment.paymentToken}`);
  assert(view.status === 200, `payment page data loads for requester (status ${view.status})`);
  assert(view.data.payment.paymentStatus === "pending", "payment status is 'pending' before pay");

  // ---------- 5+6. Pay -> confirmation emails to requester and owner ----------
  const pay = await api(cookieA, "POST", `/payments/${payment.paymentToken}/pay`);
  ...
  assert(paidRow.renterConfirmationEmailSentAt !== null, "renter confirmation email recorded");
  assert(paidRow.ownerConfirmationEmailSentAt !== null, "owner confirmation email recorded");
  ...
  const reject = await api(cookieB, "PUT", `/rentals/reject-rental/${rentalId2}`);
  assert(reject.status === 200, `rejection succeeded (status ${reject.status})`);
  const [rejectedRow] = await doQuery("SELECT status FROM rentals WHERE rentalId = ?", [rentalId2]);
  assert(rejectedRow.status === "rejected", "second rental status is 'rejected'");
  ...
  assert(threw, "email function throws on missing recipient (controller catch keeps rental)");
  const stillThere = await doQuery("SELECT COUNT(*) AS n FROM rentals WHERE rentalId IN (?, ?)", [rentalId, rentalId2]);
  assert(stillThere[0].n === 2, "both rental rows still exist after email error");

  console.log("\nE2E DONE.");
  console.log(`test rentalIds: ${rentalId}, ${rentalId2} | plate: ${licensePlate} | users: ${renterId}, ${ownerId}`);
